-- Supersedes: 20260503000000_patch_player_progress_event_id_dedup.sql
--
-- Removes p_event_id parameter entirely. Instead, dedup is handled server-side:
-- if an identical event (same username, xp_amount, xp_source, source_game) was
-- logged within the last 3 seconds, XP and season XP are skipped.
-- Presence/counters still update on duplicates.

-- Drop the 16-param version (with p_event_id) created by previous migration
DROP FUNCTION IF EXISTS public.patch_player_progress(
  text, integer, text, text, integer,
  integer, integer, integer, integer, integer, integer,
  integer, integer, integer, integer, text
);

-- Replace the original 15-param version with time-window dedup
...
begin
  ...
  -- Time-window dedup: skip XP if identical event within last 3 seconds
